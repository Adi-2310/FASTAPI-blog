...
async def get_posts(db: Annotated[AsyncSession, Depends(get_db)],
    ...
    return PaginatedPostsResponse(
        posts=[PostResponse.model_validate(post) for post in posts], # We add this since we are returning a custom response which can't be directly validated by Pydantic, therefore we need to manually validate it using model_validate method
        total=total,
        skip=skip,
        limit=limit,
        has_more=has_more,
