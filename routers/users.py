...
from schemas import PostResponse, Token, UserCreate, UserPublic, UserPrivate, UserUpdate, PaginatedPostsResponse
...
async def update_user(
    ...
    if user_update.username is not None and user_update.username.lower() != user.username.lower():
        result = await db.execute(
            select(models.User).where(models.User.username.lower() == user_update.username.lower()),
        )
        existing_user = result.scalars().first()
        if existing_user:
            raise HTTPException(
